...
def bucketExists(url: String): Boolean =
  ...
  try
    s3.headBucket(HeadBucketRequest.builder().bucket(url).build())
    true
  catch
...
/** Get the newest message from S3 from the path
  * JudgmentPackageAvailableMessage/ORIGINATOR/REFERENCE Assume that the newest
  * record is the newest object containing the last JSON object
  * @param originator
  *   A RequestJudgmentParse originator
  * @param reference
...
def getNewestJudgmentPackageAvailableMessage(
  ...
  val objects = listBucket(
    bucketName,
    "judgmentpackage.available.JudgmentPackageAvailable/" + originator + "/" + reference
  )
